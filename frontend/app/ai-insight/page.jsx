        ...
        <div className="content">
                        ...
                        <>
                            ...
                            {reasons.length > 0 && (
                                <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                                    {reasons.map((r, i) => (
                                        <div key={i} style={{ flex: 1, background: 'var(--bg3)', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: 'var(--muted2)', lineHeight: 1.6 }}>
                                            <span style={{ color: 'var(--accent)', marginRight: 4 }}>{['①','②','③'][i]}</span>{r}
                                        </div>
                                    ))}
                                </div>
                            )}
                            <button
                                ...
                                style={{ fontSize: 11, color: 'var(--muted2)', background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}
                            >
                                🔄 다시 분석
                            </button>
                        </>
            ...
            <div className="card">
                ...
                <div className="card-body">
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                        {QUICK_QUESTIONS.map(q => (
                            <button
                                ...
                                style={{ fontSize: 12, padding: '6px 12px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 20, color: 'var(--muted2)', cursor: 'pointer' }}
                            >
                                {q}
                            </button>
                        ))}
                    ...
                    <div style={{ minHeight: 200, maxHeight: 400, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
                        {chatLog.length === 0 && (
                        ...
                        {chatLog.map((m, i) => (
                            <div key={i} style={{
                                ...
                                maxWidth: '80%',
                                background: m.role === 'user' ? 'var(--green-dim)' : 'var(--bg3)',
                                border: `1px solid ${m.role === 'user' ? 'rgba(16,217,160,0.2)' : 'var(--border)'}`,
                                color: 'var(--text)',
                        ...
                        {chatLoading && (
                            <div style={{ alignSelf: 'flex-start', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '12px 12px 12px 4px', padding: '10px 14px', fontSize: 13, color: 'var(--muted2)' }}>
                                🤖 분석 중...
                            </div>
                        )}
                        <div ref={chatEndRef} />
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button
                            ...
                            style={{
                                padding: '10px 18px', background: 'var(--green)', border: 'none',
                                borderRadius: 8, color: '#000', fontSize: 13, fontWeight: 600,
                                cursor: 'pointer', opacity: chatLoading || !input.trim() ? 0.5 : 1
                            }}
                        >
                            전송
                        </button>
